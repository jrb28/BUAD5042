...
def amazon_algo(items,cart_cap):
   ...
   """
    ...
    my_username = "jrbrad"    # always return this variable as the first item
    my_nickname = 'jrbrad'   # enter a nickname here if you do not wish your WM username to be shown on the leaderboard
    ...
    ''' Create a list with the volumes and keys of each item, in that order '''
    items_list = [(v,k) for k,v in items.items()]
    items_list.sort(reverse = True)
    
    ''' Iterate through the items and pack them in descending order of volume '''
    ''' Consider packing them into the carts in descending order of valume used '''
    ''' Note that each sublist within cart_contents has two components:
          * the volume consmed in the cart
          * a list of  the item IDs in the cart '''
    for item in items_list:
        packed = False  # this Boolean variable indicates whether the item is successfully packed in an existing cart
        for i in range(len(cart_contents)):
            if cart_contents[i][0] + item[0] <= cart_cap:
                cart_contents[i][1].append(item[1])
                cart_contents[i][0] += item[0]
                packed = True # Indicate that the item is successfully packed
                cart_contents.sort(reverse = True) # order carts indescending order of fullness
                break  # item is packed, so we are finished with this item
        if packed == False:
            cart_contents.append([item[0], [item[1]]])
                
    ''' solution is complete, so put cart contents in the correct format by deleting the volume information '''
    cart_contents = [cart[1] for cart in cart_contents]
